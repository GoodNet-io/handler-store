# Standalone dev / test / build flake for the goodnet-handler-store
...
#
# goodnet-standalone-plugin: store
{
  description = "GoodNet handler plugin: store — standalone plugin flake.";
    ...
    in
    {
      ...
      devShells = forAllSystems (system: pkgs: {
        default = helpers.mkPluginDevShell pkgs {
          ...
          welcomeText = ''
  goodnet-handler-store  —  standalone plugin dev shell
    ...
    nix run .#debug      — Debug build + gdb on test_store
'';
        };
      });
